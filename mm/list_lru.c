...
bool list_lru_add_page(struct list_lru *lru, struct page *page, struct list_head *item)
{
	...
	if (list_empty(item)) {
		...
		l = list_lru_from_memcg_idx(nlru, memcg ? memcg_cache_id(memcg) : -1);
#else
		l = &nlru->lru;
		memcg = NULL;
#endif
...
{
	spin_lock(&lru->node[nid].lock);
}
EXPORT_SYMBOL_GPL(list_lru_lock);

void list_lru_unlock(struct list_lru *lru, int nid)
{
	spin_unlock(&lru->node[nid].lock);
}
EXPORT_SYMBOL_GPL(list_lru_unlock);

void list_lru_lock_irqsave(struct list_lru *lru, int nid, unsigned long *flags)
{
	spin_lock_irqsave(&lru->node[nid].lock, *flags);
}
EXPORT_SYMBOL_GPL(list_lru_lock_irqsave);

void list_lru_unlock_irqrestore(struct list_lru *lru, int nid, unsigned long *flags)
{
	spin_unlock_irqrestore(&lru->node[nid].lock, *flags);
}
EXPORT_SYMBOL_GPL(list_lru_unlock_irqrestore);
