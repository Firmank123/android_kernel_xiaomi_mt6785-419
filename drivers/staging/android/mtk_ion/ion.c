	...
	if (heap->type == ION_HEAP_TYPE_MULTIMEDIA_SEC) {
		...
		if (total_size < 0) {
			IONMSG("heap_id:%u underflow!, total_now[%lld--%lld]\n",
			       heap->id, (long long)total_size,
			       (long long)atomic64_read(&client->total_size[SECURE_HEAP]));
		...
		if (total_size < 0) {
			IONMSG("heap_id:%u underflow!, total_now[%lld--%lld]\n",
			       heap->id, (long long)total_size,
			       (long long)atomic64_read(&client->total_size[SYSTEM_HEAP]));
		...
		if (total_size < 0) {
			IONMSG("heap_id:%u underflow!, total_now[%lld--%lld]\n",
			       heap->id, (long long)total_size,
			       (long long)atomic64_read(&client->total_size[NORMAL_HEAP]));
