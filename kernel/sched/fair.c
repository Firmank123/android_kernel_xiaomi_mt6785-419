 ...
 *
 * (default: 6.5ms * (1 + ilog(ncpus)), units: nanoseconds)
 */
unsigned int sysctl_sched_latency			= 6500000ULL;
unsigned int normalized_sysctl_sched_latency		= 6500000ULL;
 ...
 *
 * (default: 0.8 msec * (1 + ilog(ncpus)), units: nanoseconds)
 */
...
util_est_dequeue(struct cfs_rq *cfs_rq, struct task_struct *p, bool task_sleep)
{
	 ...
	 */
	if ((ue.enqueued + (SCHED_CAPACITY_SCALE / 100)) < task_runnable(p))
		goto done;
...
static inline void update_misfit_status(struct task_struct *p, struct rq *rq)
{
	...
	if (!p || (p->nr_cpus_allowed == 1) ||
	    (arch_scale_cpu_capacity(NULL, cpu) == p->max_allowed_capacity) ||
	    task_fits_cpu(p, cpu)) {
...
{
	p->max_allowed_capacity = arch_scale_cpu_capacity(NULL, cpumask_first(&p->cpus_allowed));
}
